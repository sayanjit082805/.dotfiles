...
local color                    = require("mods.bling.helpers.color")
...
local function select_app(self, x, y)
    ...
    if widgets then
        ...
        if self._private.active_widget ~= nil then
            ...
            if name_widget then
                name_widget.markup = string.format("<span foreground='%s'>%s</span>", self.app_name_selected_color,
                    name_widget.text)
            end
            ...
            if generic_name_widget then
                generic_name_widget.markup = string.format("<i><span weight='300'foreground='%s'>%s</span></i>",
                    self.app_name_selected_color, generic_name_widget.text)
            end
        end
    end
end
...
local function unselect_app(self)
    if self._private.active_widget ~= nil then
        ...
        local name_widget = self._private.active_widget:get_children_by_id("name")[1]
        if name_widget then
            name_widget.markup = string.format("<span foreground='%s'>%s</span>", self.app_name_normal_color,
                name_widget.text)
        end
        local generic_name_widget = self._private.active_widget:get_children_by_id("generic_name")[1]
        if generic_name_widget then
            generic_name_widget.markup = string.format("<i><span weight='300'foreground='%s'>%s</span></i>",
                self.app_name_normal_color, generic_name_widget.text)
        end
...
local function search(self, text)
    ...
    self._private.pages_count = math.ceil(math.max(1, #self._private.matched_entries) /
...
local function generate_apps(self)
    ...
    local icon_theme = require("mods.bling.helpers.icon_theme")(self.icon_theme, self.icon_size)
                ...
                if icon ~= "" or self.skip_empty_icons == false then
                    if icon == "" then
                        ...
                        elseif self.default_app_icon_path ~= nil then
                            ...
                            icon = icon_theme:choose_icon({ "application-all", "application", "application-default-icon",
                                "app" })
                        end
                    end

                    local desktop_app_info = Gio.DesktopAppInfo.new(app_info.get_id(app))
                    local terminal = Gio.DesktopAppInfo.get_string(desktop_app_info, "Terminal") == "true" and true or
                        false
...
function app_launcher:toggle()
    ...
    if screen.app_launcher and screen.app_launcher.visible then
        self:hide()
        awesome.emit_signal("bling::app_launcher::visibility", false)
    else
        self:show()
        awesome.emit_signal("bling::app_launcher::visibility", true)
    end
end

local function sessionctl()
    ...
    local cr_btn           = function(text_cc, icon_cc, color, command)
        local i = wibox.widget {
            ...
            markup = helpers.colorize_text(icon_cc, beautiful.fg_color .. "99"),
            widget = wibox.widget.textbox()
        }
        ...
        button:connect_signal("mouse::leave", function()
            i.markup = helpers.colorize_text(icon_cc, beautiful.fg_color .. "99")
        end)
    ...
    local icons            = {
        -- poweroff = "",
        poweroff = helpers.colorize_text("", beautiful.red_color),
        -- reboot   = "",
        reboot = helpers.colorize_text("", beautiful.green_color),
        -- exit     = ""
        exit = helpers.colorize_text("", beautiful.accent)
    }
...
local time_wid = wibox.widget {
    {
        ...
        format = helpers.colorize_text("%H", beautiful.fg_color .. "99"),
        align = "center",
        valign = "center",
    ...
    {
        font = beautiful.font_var .. "Bold 12",
        format = helpers.colorize_text("%M", beautiful.fg_color .. "99"),
        align = "center",
        valign = "center",
        widget = wibox.widget.textclock
    },
...
local function new(args)
    ...
    args.reverse_sort_alphabetically = args.reverse_sort_alphabetically ~= nil and args.reverse_sort_alphabetically or
        false
    args.select_before_spawn = args.select_before_spawn == nil and true or args.select_before_spawn
    args.hide_on_left_clicked_outside = args.hide_on_left_clicked_outside == nil and true or
        args.hide_on_left_clicked_outside
    args.hide_on_right_clicked_outside = args.hide_on_right_clicked_outside == nil and true or
        args.hide_on_right_clicked_outside
    args.hide_on_launch = args.hide_on_launch == nil and true or args.hide_on_launch
    args.try_to_keep_index_after_searching = args.try_to_keep_index_after_searching ~= nil and
        args.try_to_keep_index_after_searching or false
    ...
    args.app_normal_hover_color = args.app_normal_hover_color or
    ...
    args.app_selected_hover_color = args.app_selected_hover_color or
        (color.is_dark(args.app_normal_color) or color.is_opaque(args.app_normal_color)) and
    ...
    ret._private.widget = awful.popup
        {
            ...
            placement = function(w)
                awful.placement.bottom_left(w, { honor_workarea = true, margins = beautiful.useless_gap * 2 })
            end,
            shape = ret.shape,
            bg = ret.background,
            widget =
                ...
                {
                    {
                        ...
                        margins = { left = dpi(15), right = dpi(15), top = dpi(20), bottom = dpi(20) },
                        widget = wibox.container.margin
                    },
                    widget = wibox.container.background,
                    bg = beautiful.bg_2,
