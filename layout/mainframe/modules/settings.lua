...
local createButton = function(label, labelfalse, text, signal, cmd)
  ...
  awesome.connect_signal('signal::' .. signal, function(status)
    if status then
      settingbuttonlabel.markup = helpers.colorize_text(label, beautiful.pri)
      ...
      settingbuttonlabel.markup = helpers.colorize_text(labelfalse, beautiful.fg .. 'cc')
...
local wifibtn      = createButton("󰤨", "󰤮", 'Wifi', 'wifi', "~/.config/awesome_backup/scripts/wifi --toggle")
local bluetooth    = createButton("󰂯", "󰂲", 'Bluetooth', 'bluetooth',
  "~/.config/awesome_backup/scripts/bluetooth --toggle")
...
local airplane     = createButton("󰀝", "󰀞", 'Airplane', 'airplane',
  "~/.config/awesome_backup/scripts/airplanemode --toggle")
  ...
  {
    {
      {
        font = beautiful.font_var .. " 11",
        markup = helpers.colorize_text('Quick Actions', beautiful.fg3),
        valign = "center",
        widget = wibox.widget.textbox,
      },
      {
