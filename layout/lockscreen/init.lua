  ...
  awful.spawn.easy_async_with_shell(
    ...
    function(stdout)
      if stdout:match('off') then
        label.markup = "Type Password To Unlock"
      else
...
local reset = function(f)
  ...
  header:get_children_by_id('arc')[1].colors = { not f and beautiful.red_color or beautiful.fg_color }
end
...
local function grab()
  local grabber = awful.keygrabber {
    ...
    keypressed_callback  = function(_, _, key, _)
      ...
      if #key == 1 then
        header:get_children_by_id('arc')[1].colors = { beautiful.accent }
      ...
      elseif key == "BackSpace" then
        ...
        input = input:sub(1, -2)
        if #input == 0 then
          header:get_children_by_id('arc')[1].colors = { beautiful.green_color }
    ...
    keyreleased_callback = function(self, _, key, _)
      ...
      if key == 'Return' then
        if auth(input) then
          ...
          header:get_children_by_id('arc')[1].colors = { beautiful.red_color }
          reset(false)
          grab()
          input = ""
        end
    ...
    {
      {
        {
          {
            font = beautiful.font_var .. " Medium 110",
            format = "%H:%M",
            halign = "center",
            valign = "center",
          ...
          {
            font = beautiful.font_var .. " Light 28",
            format = "%a, %d %B",
            halign = "center",
            valign = "center",
            widget = wibox.widget.textclock
